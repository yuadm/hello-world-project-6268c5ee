...
export function CochildminderFormSection2({ formData, setFormData, validationErrors = {} }: Props) {
  ...
  const coverage = useMemo(() => {
    if (!formData.currentAddress.moveIn) return null;
    ...
    return calculateAddressHistoryCoverage(
      { moveIn: formData.currentAddress.moveIn },
      historyForCalc
    );
  }, [formData.currentAddress.moveIn, formData.addressHistory]);
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          ...
          <RKPostcodeLookup
            ...
            onChange={(postcode) => setFormData(prev => ({ 
              ...prev, 
              currentAddress: { ...prev.currentAddress, postcode } 
            }))}
            onAddressSelect={handleCurrentAddressSelect}
            error={validationErrors["currentAddress.postcode"]}
          />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <div className="md:col-span-2">
                <RKInput
                  ...
                  error={validationErrors["currentAddress.line1"]}
                />
              </div>
              <div className="md:col-span-2">
                <RKInput
                  ...
                  onChange={(e) => setFormData(prev => ({ 
                    ...prev, 
                    currentAddress: { ...prev.currentAddress, line2: e.target.value } 
                  }))}
                />
              </div>
              <RKInput
                ...
                onChange={(e) => setFormData(prev => ({ 
                  ...prev, 
                  currentAddress: { ...prev.currentAddress, town: e.target.value } 
                }))}
                error={validationErrors["currentAddress.town"]}
              />
              <RKInput
                id="postcode"
                label="Postcode"
                ...
                onChange={(e) => setFormData(prev => ({ 
                  ...prev, 
                  currentAddress: { ...prev.currentAddress, postcode: e.target.value } 
                }))}
              />
            </div>
          )}
          ...
          <RKInput
            ...
            required
            className="max-w-[200px]"
            value={formData.currentAddress.moveIn}
            onChange={(e) => setFormData(prev => ({ 
              ...prev, 
              currentAddress: { ...prev.currentAddress, moveIn: e.target.value } 
            }))}
            error={validationErrors["currentAddress.moveIn"]}
          />
        </div>
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-600 -mt-2">
            Please add all previous addresses from the last 5 years.
          </p>
          ...
          <div className="space-y-4">
            {formData.addressHistory.map((addr, index) => (
              <div key={index} className="p-5 bg-white border border-gray-200 rounded-xl space-y-4">
